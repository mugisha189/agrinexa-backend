...
@router.post(
    "/register", response_model=AuthResponseModel, status_code=status.HTTP_201_CREATED
)
...
@router.post(
    "/forgot-password",
    response_model=AuthResponseModel2,
...
async def forgot_password(cred: ForgotPasswordModel):
    ...
    try:
        await send_email(
            ...
            render_template(
                "forgot-password.html", verification_code=verification_code
            ),
        )
